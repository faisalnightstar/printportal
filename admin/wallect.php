							...
							<div class="row">
							   <?php
								...
								if(isset($_POST['submit'])) {	
									...
									$q = "";
									$q = "SELECT findwallet FROM tbluser where  userid='".$_SESSION['userid']."'";
									$rr = mysqli_query($connection,$q);
									$rwr = mysqli_fetch_assoc($rr);
									$findwallet=$rwr['findwallet'];
                                    if ($_SESSION['userid']==1){
										$findwallet=1000000000000000000000000;
									}
									...
									if ($findwallet>$ptr){
										...
										if($f['findwallet'] == '')
										{
										$sql = "update tbluser SET findwallet= 0 + ".$ptr." where userid=".$touserid."";
										}
										else 
										{
											$sql = "update tbluser SET findwallet= findwallet + ".$ptr." where userid=".$touserid."";
										}
										...
										if($fp['findwallet'] == '')
										{
										$sql = "update tbluser SET findwallet= 0 - ".$ptr." where userid=".$_SESSION['userid']."";
										}
										else 
										{
											$sql = "update tbluser SET findwallet=findwallet - ".$ptr." where userid=".$_SESSION['userid']."";
										}
								...
								<form method="post" onSubmit="return validation();" action="" style="width:100%" novalidate>
									...
									<div class="row dgnform" style="padding:10px;">
									    ...
									    $result="";
										$result =mysqli_query($connection,"SELECT `userid`,`usertype`,`fullname`,  `usertype`, `panimage`,`aadharimage`,`loginname`,`status`,  `pswrd`, `emailid`, `adhaarno`, `mobileno`,  `address`, `logdate`, `cityname`, `statename`, `refrenceid`, `remarks`, `findwallet`,`ispaid` FROM `tbluser` where refrenceid='".$_SESSION['userid']."' or remarks='".$url['remarks']."' and findwallet < 2 ORDER BY fullname");
									    }
												    else {
														$result = mysqli_query($connection,"SELECT fullname,userid,loginname,mobileno FROM tbluser where  refrenceid='".$_SESSION['userid']."' and userid<>'".$_SESSION['userid']."' and userid>1 and findwallet < 200000000000000000  ORDER BY fullname"); 
													}
													?>
