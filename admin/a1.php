...
// Fetch user's wallet balance
    $fetch_wallet_sql = "SELECT walletamount FROM tbluser WHERE userid = '".$_SESSION['userid']."'";
    $wallet_result = mysqli_query($connection, $fetch_wallet_sql);
    $wallet_row = mysqli_fetch_assoc($wallet_result);
    $walletamount = $fetch['walletamount'];
    ...
    if ($walletamount >= $fee) {
        ...
        curl_setopt_array($curl, array(
            CURLOPT_URL => 'https://test.axenapi.co.in/Dashboard/Verify_api/aad/ne_aad.php',
            ...
            CURLOPT_POSTFIELDS => array('bioenc' => $bio, 'aadhar' => $aadhar,'api' => 'CjtGHoUc-oaM6-nCcw-GjG9-sfpHS1wzYiHqoGU5'),
        ));
        ...
        if ($status === 'Success') {
            $name = $json["name"];
            $debit_fee =  $walletamount - $fee;                               
            $sql = "update tbluser SET walletamount = '$debit_fee' where userid='".$_SESSION['userid']."'";
            $abs = mysqli_query($connection, $sql);
        } else {
            $errCodeDesc = $json['error'];
            echo "<script>alert('failed! $errCodeDesc')</script>";
        }
    } else {
        // Insufficient balance, show alert
        echo "<script>alert('Insufficient balance. Please recharge your wallet.')</script>";
...
elseif ( $localaddress == "" ) {
  ...
  if ( $b[ 'aadharno' ] == $aadharno ) {
    ...
    $a = mysqli_query( $connection, "SELECT aadharno,aadharname FROM aadharauto Where aadharno='" . $_SESSION[ "AADHAARNO" ] . "'" );
    ...
    if($fetch['prime'] < 2){
    $walletpaisacut = $pricing['adprint'];
    $sql = "update tbluser SET walletamount= walletamount - $walletpaisacut where userid='" . $_SESSION[ 'userid' ] . "'";
    }else{}
    $abs = mysqli_query( $connection, $sql );
    ...
    $qu = "INSERT INTO `tbltrans`(`userid`, `username`, `transdate`, `transqty`, `transtype`, `touserid`, `tousername`, `remark`, `loginid`, `logdate`)
                                    ...
                                    VALUES ('" . $_SESSION[ 'userid' ] . "','" . $_SESSION[ 'username' ] . "',now(),10,'Dr','0','Aadhaar Create','" . $remark . "','" . $_SESSION[ 'userid' ] . "',now())";
              ...
              <div class="row">
                ...
                <div class="col-md-4">
                  <div class="form-group">
                    ...
                    <input class="  form-control " maxlength="12" readonly id="aadharno" name="aadharno" type="text"  value="<?php echo $aadharno;?>" required>
                  </div>
                </div>
                <div class="col-md-4">
                  <div class="form-group">
