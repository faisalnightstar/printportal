...
// Fetch user's wallet balance
    ...
    if ($findwallet >= $fee) {
        ...
        curl_setopt_array($curl, array(
            CURLOPT_URL => 'https://api.controlway.co.in/serviceApi/V1/aadhaarVerification.php',
            ...
            CURLOPT_POSTFIELDS => array('bioenc' => $bio, 'aadhar' => $aadhar,'apiKey' => 'CjtGHoUc-oaM6-nCcw-GjG9-sfpHS1wzYiHqoGU5'),
        ));
        ...
        if ($status === 'Success') {
            ...
            $sql = "update tbluser SET findwallet = '$debit_fee' where userid='".$_SESSION['userid']."'";
            $abs = mysqli_query($connection, $sql);
        } else {
            echo "<script>alert('$error_api')</script>";  // Data Not Fetched
        }
    } else {
...
if ( isset( $_POST[ 'savedata' ] ) ) {
  ...
  } else if ( $fetch[ 'aadharpoint' ] > $fetch[ 'findwallet' ] ) {
...
elseif ( $localaddress == "" ) {
  ...
  if ( $b[ 'aadharno' ] == $aadharno ) {
    ...
    $sql = "update tbluser SET findwallet= findwallet - $walletpaisacut where userid='" . $_SESSION[ 'userid' ] . "'";
    }else{}
    $abs = mysqli_query( $connection, $sql );
