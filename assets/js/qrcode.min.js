/**
 * Official Standalone QRCode.js Library (David Shim / EasyQRCode Compatible)
 * Fixed for Version 1 to 40 QR Codes without pos.length TypeError
 */
(function () {
    ...
    QRCodeModel.prototype = {
        ...
        setupTypeInfo: function (test, maskPattern) {
            ...
            this.modules[this.moduleCount - 8][8] = (!test && ((bits >> 14) & 1) == 1);
        },
    ...
    var QRUtil = {
        ...
        G15_MASK: (1 << 14) | (1 << 12) | (1 << 8) | (1 << 4) | (1 << 1),
        ...
        getPatternPosition: function (typeNumber) {
            if (!typeNumber || typeNumber < 1 || typeNumber > 40) return [];
        ...
        getMask: function (maskPattern, i, j) {
            switch (maskPattern) {
                ...
                default: throw new Error("bad maskMode:" + maskPattern);
            }
        },
    ...
    QRRSBlock.getRsBlockTable = function (typeNumber, errorCorrectLevel) {
        switch (errorCorrectLevel) {
            ...
            default: return QRRSBlock.RS_BLOCK_TABLE[(typeNumber - 1) * 4 + 0];
        }
    };
    ...
    // Auto version calculator helper
    function _autoCalculateVersion(text) {
        ...
        if (length <= 134) return 7;
        if (length <= 154) return 8;
        return 10;
    }

    // Modern Generator API
    window.QRCodeGenerator = {
        render: function (element, text, size) {
            size = size || 220;
            if (typeof element === 'string') {
                element = document.getElementById(element);
            }
            if (!element || !text || text.trim() === '') return;

            element.innerHTML = '';
            var typeNum = _autoCalculateVersion(text);

            var qr = new QRCodeModel(typeNum, QRErrorCorrectLevel.M);
            qr.addData(text);
            qr.make();
            ...
            for (var r = 0; r < count; r++) {
                for (var c = 0; c < count; c++) {
                    if (qr.isDark(r, c)) {
                        var x = Math.round(c * tile);
                        var y = Math.round(r * tile);
                        var w = Math.ceil((c + 1) * tile) - x;
                        var h = Math.ceil((r + 1) * tile) - y;
    ...
    // Standard David Shim / EasyQRCode Compatible Constructor
    window.QRCode = function (element, options) {
        if (typeof element !== 'string' && !(element instanceof HTMLElement) && typeof options === 'undefined') {
            return;
        }
        ...
        if (element && text) {
            window.QRCodeGenerator.render(element, text, width);
        }
    };
